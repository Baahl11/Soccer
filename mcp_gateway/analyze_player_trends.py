...
def main():
 ...
 for path in sorted(glob.glob(os.path.join(a.history_dir,'*.jsonl'))):
  for line in open(path,encoding='utf-8'):
   ...
   for e in t.get('events') or []:
    pr=e.get('player_trends_research') or {};fid=(e.get('fixture') or {}).get('fixture_id')
    if pr.get('status')!='RESEARCH_ONLY_PLAYER_FIXTURE_STATS' or not fid:continue
    seen[int(fid)]={'fixture_id':int(fid),'kickoff':(e.get('fixture') or {}).get('kickoff'),'teams':pr.get('teams') or []}
 ...
 for pid,rs in rows.items():
  ...
  out.append(item)
 report={'schema_version':'1.0.0','status':'RESEARCH_ONLY_PLAYER_TRENDS','generated_at_utc':datetime.now(timezone.utc).isoformat(),'decision_weight':0.0,'unique_fixtures_captured':len(seen),'unique_players':len(out),'players_with_5plus_matches':sum(x['matches_captured']>=5 for x in out),'policy':['Only persisted verified fixture/player captures are used.','Fixture IDs are deduplicated to avoid repeated scheduler snapshots.','Last-5/10/20 are descriptive until sufficient OOS validation exists.','No player trend can change classification, tier, stake, or bet eligibility.'],'players':sorted(out,key=lambda x:(-x['matches_captured'],str(x['name'])))}
