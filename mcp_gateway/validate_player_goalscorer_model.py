...
def main() -> None:
    ap = argparse.ArgumentParser(description="Structural sanity validation for research-only goalscorer model.")
    ...
    ap.add_argument("--output", default="soccer_edge_state/analysis/player_goalscorer_model_sanity.json")
    args = ap.parse_args()
    ...
    report = {
        ...
        "opponent_goals_context_available": _num(global_context.get("avg_team_goals")) is not None,
        "global_avg_team_goals": global_context.get("avg_team_goals"),
        "validation_scope": "STRUCTURAL_ONLY_NOT_OOS_PERFORMANCE",
        "checks": [
            "anytime scoring probability is bounded [0,1]",
            "P(2+ goals) <= P(1+ goal)",
            "fair no-vig anytime decimal is finite when probability is nonzero",
