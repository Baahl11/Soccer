...
def _market_snapshot(best: Any) -> dict[str, Any] | None:
    if not isinstance(best, dict) or not best: return None
    return {k: (_f(best.get(k)) if k in {"line","decimal_price","p_raw","p_shrunk","p_breakeven","p_market_fair","prob_edge_pp","ev_pct"} else best.get(k)) for k in ("family","market","selection","line","decimal_price","bookmaker","p_raw","p_shrunk","p_breakeven","p_market_fair","prob_edge_pp","ev_pct","tier")}


def _lineup_snapshot(lineups: Any) -> dict[str, Any] | None:
    if not isinstance(lineups, dict) or not lineups: return None
...
def _raw_snapshot(raw: Any) -> dict[str, Any] | None:
    ...
    if isinstance(shadow,dict): out["relative_strength_shadow"]=shadow
    return out


def _shortlist_snapshot(s: Any) -> dict[str, Any] | None:
    if not isinstance(s,dict) or not s: return None
...
def _result_snapshot(event: dict[str,Any], fixture: dict[str,Any]) -> dict[str,Any] | None:
    ...
    if fixture.get("status") in {"FT","AET","PEN"} and (fixture.get("goals") or {}).get("home") is not None: return {"goals":fixture.get("goals"),"score":fixture.get("score"),"status":fixture.get("status")}
    return None
...
def build_rows(history_dir: str):
    rows=[]; seen=set(); bad_lines=0; ticks=0
        ...
        with open(path,encoding="utf-8") as fh:
            for line in fh:
                ...
                for event in tick.get("events") or []:
                    fixture=event.get("fixture") or {}; fid=fixture.get("fixture_id")
                    if not fid: continue
                    md=event.get("market_decision") or {}; coverage=event.get("coverage") or {}
                    row={"generated_at_local":tick.get("generated_at_local"),"generated_at_utc":tick.get("generated_at_utc"),"timezone":tick.get("timezone") or "America/Mexico_City","service_version":tick.get("version"),"model_version":tick.get("model_version"),
                    ...
                    if row["event_key"] in seen: continue
                    seen.add(row["event_key"]); rows.append(row)
    rows.sort(key=lambda r:(r.get("generated_at_local") or "",r["fixture_id"],r.get("stage") or ""))
    stage_counts=Counter(str(r.get("stage") or "UNKNOWN") for r in rows); class_counts=Counter(str(r.get("classification") or "UNKNOWN") for r in rows); signal_counts=Counter(); fixtures=set(); finals=set(); with_market=with_raw=with_lineups=with_shadow=0
    for r in rows:
        fixtures.add(r["fixture_id"]); finals.add(r["fixture_id"]) if r.get("result") else None
        with_market+=bool(r.get("best_market")); with_raw+=bool(r.get("raw_projection")); with_lineups+=bool(r.get("lineups")); with_shadow+=bool((r.get("raw_projection") or {}).get("relative_strength_shadow"))
        for track in (r.get("sporting_shortlist") or {}).get("tracks") or []: signal_counts[str(track)]+=1
    summary={"schema_version":"1.1.0","source":"soccer_edge_state/history/*.jsonl","timezone_basis":"America/Mexico_City","ticks_read":ticks,"bad_lines":bad_lines,"rows":len(rows),"unique_fixtures":len(fixtures),"fixtures_with_final_result":len(finals),"rows_with_raw_projection":with_raw,"rows_with_relative_strength_shadow":with_shadow,"rows_with_market":with_market,"rows_with_lineups":with_lineups,"stage_counts":dict(sorted(stage_counts.items())),"classification_counts":dict(sorted(class_counts.items())),"sport_signal_counts":dict(sorted(signal_counts.items())),"first_generated_at_local":rows[0].get("generated_at_local") if rows else None,"last_generated_at_local":rows[-1].get("generated_at_local") if rows else None}
    return rows,summary


def main():
    p=argparse.ArgumentParser(); p.add_argument("--history-dir",default="soccer_edge_state/history"); p.add_argument("--output",default="soccer_edge_state/analysis/signal_ledger.jsonl"); p.add_argument("--summary-output",default="soccer_edge_state/analysis/signal_ledger_summary.json"); a=p.parse_args()
    rows,summary=build_rows(a.history_dir); os.makedirs(os.path.dirname(a.output),exist_ok=True)
    with open(a.output,"w",encoding="utf-8") as fh:
        for row in rows: fh.write(json.dumps(row,ensure_ascii=False,separators=(",",":"))+"\n")
    with open(a.summary_output,"w",encoding="utf-8") as fh: json.dump(summary,fh,ensure_ascii=False,indent=2,sort_keys=True); fh.write("\n")
