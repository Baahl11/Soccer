...
async def internal_player_props_phase15_coverage_audit(request: Request) -> Response:
    try:
        _github_oidc_claims(
            request,
            {".github/workflows/player-props-phase15-coverage-audit.yml"},
        )
    except Exception as exc:
        return JSONResponse({"error": "unauthorized", "detail": str(exc)[:200]}, status_code=401)
...
@mcp.custom_route("/internal/player-trend-registry-backfill-v4/run", methods=["POST"])
async def internal_player_trend_registry_backfill_v4_run(request: Request) -> Response:
    try:
        _github_oidc_claims(
            request,
            {".github/workflows/player-trend-registry-backfill.yml"},
        )
    except Exception as exc:
        return JSONResponse({"error": "unauthorized", "detail": str(exc)[:200]}, status_code=401)
    ...
    try:
        result = await player_trend_registry_backfill_v4.run_backfill(
            lookback_days=lookback_days,
            max_fixtures=max_fixtures,
        )
        return JSONResponse(result)
