...
def _opponent_modifier(
    ...
    allowed = _num(l10.get("avg_opponent_sot"))

    if global_sot is None or global_sot <= 0 or allowed is None or n < 5:
        return {
            "factor": 1.0,
            "status": "NEUTRAL_INSUFFICIENT_OPPONENT_SOT_HISTORY",
            "opponent_last10_n": n,
            "opponent_avg_sot_allowed": allowed,
            "global_avg_team_sot": global_sot,
        }

    raw = allowed / global_sot
    ...
    return {
        ...
        "status": "SHRUNK_OPPONENT_SOT_SUPPRESSION",
        "opponent_last10_n": n,
        "opponent_avg_sot_allowed": allowed,
        "global_avg_team_sot": global_sot,
...
def _formation_context(event: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "factor": 1.0,
        "reason": "NO_PLAYER_SOT_SPECIFIC_FORMATION_RESIDUAL_OOS_MODEL",
    }
...
def build(
    ...
    for team in lineups.get("teams") or []:
        ...
        for starter in team.get("starters") or []:
            ...
            players.append({
                ...
                "position": starter.get("pos"),
                "confirmed_starter": True,
                "status": "LIVE_RESEARCH_SOT_DISTRIBUTION",
                "sample_band": sample_band,
                "expected_minutes_if_confirmed_starter": round(expected_minutes, 3),
                "base_posterior_mean_sot_per90": sot.get("posterior_mean_per90"),
                ...
                "expected_sot": round(mean_count, 6),
                "lines": lines,
    ...
    return {
        "schema_version": SCHEMA_VERSION,
        "fixture_id": fixture.get("fixture_id"),
        "status": "LIVE_RESEARCH_SOT_PROPS",
        "model": "CONFIRMED_STARTER_MINUTES_X_SHRUNK_SOT_RATE_X_OPPONENT_SOT_SUPPRESSION__NB_PREDICTIVE_v0.1",
        "players": players,
        ...
        "calibration_gate": {
            ...
            "requires": [
                "walk-forward Brier/log-loss by SOT line and probability bucket",
                "count calibration and dispersion review by position/competition",
                "confirmed starter and minutes calibration",
                "observed sportsbook player-SOT line and price",
                "true CLV before any production promotion",
                "SOT-specific formation residual OOS evidence before non-neutral formation modifier",
            ],
        },
        "policy": "EXACT SOT HALF-LINE PROBABILITIES ARE FAIR RESEARCH ESTIMATES ONLY; NO MARKET PRICE OR EV MEANS NO BET/LEAN/GALAXY LEG; FORMATION FACTOR REMAINS 1.0 UNTIL SOT-SPECIFIC OOS LIFT",
    }
...
def attach(payload: dict[str, Any]) -> dict[str, int | bool]:
    ...
    for event in payload.get("events") or []:
        ...
        event["player_sot_intelligence"] = intel
        if intel.get("status") == "LIVE_RESEARCH_SOT_PROPS":
            modeled_events += 1
