...
| 43 | Galaxy Multi | ✅ market-backed component legs | ✅ rolling pool v0.5 across natural ticks | ✅ | 🔴 final without verified parlay quote | partial | 🟢 IMPROVED | Rolling leg pool, same-book component reference, freshness, edge and one-leg-per-fixture gates are live. Expand only with production-valid families; exact combined quote still blocks final BET. |
...
| 45 | Exact SGP quote | ⚫ current GalaxyParlay fixture contract exposes individual predictions/odds only | — | 🔴 | required | — | ⚫ EXTERNAL CONFIRMED | Current client endpoint has no combined-SGP quote contract. Need sportsbook/provider endpoint returning the actual same-game parlay price; component-product remains reference only and can never substitute for final quote. |
...
- #45 Exact SGP quote: current GalaxyParlay persisted-fixture client exposes individual odds/predictions only; no combined quote endpoint is present in the current contract.
...
#44 Galaxy SGP correlation — build a score-matrix joint probability engine for same-game combinations whose legs are deterministic functions of full-time score; never multiply same-game marginals.