...
This is the authoritative resumption point with v3.60.0 deployed LIVE on Render. Persisted natural validation remains asynchronous and must not be forced with a manual tick.
...
- **#44 v3.60 Score-Matrix Expansion:** deployed LIVE on Render; worker targets v3.60.0. Research-only joint coverage expands to 1X2, Home/Away Team Totals, Correct Score and true half-goal Asian Handicap using direct score-matrix intersection. DNB and integer/quarter Asian Handicap use settlement-aware score-matrix diagnostics so pushes/half-settlements are not mislabeled as binary outcomes. Exact fresh observed component quotes remain required. New v3.60 research candidates are not merged into the primary Galaxy feed and cannot become BET.
...
### Resume sequence
1. When the next natural persisted state arrives, verify version >=3.60.0, FT Goals exact ladder alignment, provider-timestamp quote freshness, distinct EARLY_RESEARCH/T-90 lifecycle, and v3.60 research expansion fields. Do not trigger a manual tick.
2. Continue #44 validation and score-derived correlation hardening; keep v3.60 research candidates outside the primary Galaxy feed until OOS/calibration evidence exists.
3. Keep corners/cards/player/GK same-game correlations blocked until their individual models and joint dependencies are defensible.
4. After this #44 block, repair the transverse promotion-review false-ready classifier.
