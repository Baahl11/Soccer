...
| 44 | Galaxy SGP correlation | ✅ score-derived inputs for FT Goals/BTTS/DC + 1X2/Team Totals/Correct Score/DNB/AH research bridges | ✅ v3.60 direct score-matrix intersections + settlement-aware DNB/AH diagnostics | 🟡 v3.60 expanded live research | 🔴 | ⏳ | 🟡 LIVE RESEARCH EXPANDED v3.60 | Binary score-derived families use direct matrix intersection; DNB and integer/quarter AH stay settlement-aware. Same-game marginals are never multiplied. Corners/cards/player/GK correlations remain blocked pending defensible joint models + OOS calibration. |
...
This is the authoritative resumption point with the worker now targeting v3.60.0. Recent operational patches are deployed sequentially; persisted natural validation remains asynchronous and must not be forced with a manual tick.
...
- **v3.58.1 deploy reconciliation:** Render was reconciled to the actual `soccer-edge-mcp-v1` branch instead of the misleading globally-recent commit chain. The first reconciled deploy went LIVE; natural state observed at that time predated the deploy, so natural validation remained pending.
- **v3.58.2 FT Goals ladder ↔ Galaxy:** deployed LIVE. Galaxy FT_GOALS legs now use the canonical `market_decision` TOTAL ladder (same exact observed line, bookmaker and `p_shrunk`) instead of a second fixed synthetic marginal ladder. The score matrix remains the same-game joint-correlation engine only. Rolling Galaxy legs consume this same aligned source.
- **v3.58.3 quote freshness:** deployed LIVE. Quote age is anchored to `provider_update`, not worker capture time. Galaxy odds and API-Football fallback odds fail closed when provider timestamps are stale/missing/unparseable; per-decision BET/LEAN rows are downgraded to WATCH when their specific quote is stale. Rolling leg/candidate registries expire stale provider quotes instead of rejuvenating them on recapture.
- **v3.59 Operational Market Lifecycle:** deployed LIVE. `EARLY_RESEARCH` is now distinct from the real `T-90` processed-stage key. EARLY_RESEARCH and true T-90 may capture price only after SPORT-FIRST shortlist and remain research-only/WATCH. T-40/T-20/T-10 release the elastic cap only through the existing fresh-quote + availability/XI-GK gates; no new model weights or canonical probability thresholds were introduced.
- **#44 v3.60 Score-Matrix Expansion:** worker now targets v3.60.0. Research-only joint coverage expands to 1X2, Home/Away Team Totals, Correct Score and true half-goal Asian Handicap using direct score-matrix intersection. DNB and integer/quarter Asian Handicap use settlement-aware score-matrix diagnostics so pushes/half-settlements are not mislabeled as binary outcomes. Exact fresh observed component quotes remain required. New v3.60 research candidates are not merged into the primary Galaxy feed and cannot become BET.
- **#43 Galaxy Multi:** distinct-fixture component odds may be multiplied only when all legs use the same verified fresh bookmaker snapshot/reference. Bet365 is preferred when present and viable. Final executable placement/quote verification remains required.
- **#45 SGP pricing:** internal fair probability, fair decimal/american and minimum playable price are model-derived from the joint model; executable Bet365 Bet Builder price remains a separate market input. Same-game component odds are screening references only and are never multiplied into an executable SGP price.
- **Non-score #44 families remain blocked:** corners/team corners/cards/team cards/player props/GK saves still require defensible family-specific joint/correlation models and OOS calibration before entering production correlation logic.
- **Calibration governance issue remains transverse:** `promotion_review` can still mislabel sanity-only/data-blocked families as ready for manual promotion review. Fix this after the current #44 block; it does not justify interrupting the user-ordered sequence.
...
- **No manual natural-validation ticks.** Natural validation is asynchronous and non-blocking; inspect the next persisted scheduler state when it arrives.

- **Galaxy postgame grading:** `mcp_gateway/analyze_galaxy_history.py` remains wired into nightly analysis. It deduplicates logical Galaxy combinations across ticks, grades supported legs against persisted final results, reports MULTI and SGP separately, and treats component-product profit as hypothetical reference bookkeeping only—not official bankroll ROI.

### Resume sequence
1. Deploy the v3.60 branch head and confirm Render is LIVE.
2. When the next natural persisted state arrives, verify version >=3.60.0, FT Goals exact ladder alignment, provider-timestamp quote freshness, distinct EARLY_RESEARCH/T-90 lifecycle, and v3.60 research expansion fields. Do not trigger a manual tick.
3. Continue #44 validation and score-derived correlation hardening; keep v3.60 research candidates outside the primary Galaxy feed until OOS/calibration evidence exists.
4. Keep corners/cards/player/GK same-game correlations blocked until their individual models and joint dependencies are defensible.
5. After this #44 block, repair the transverse promotion-review false-ready classifier.
