...
| 45 | Exact SGP quote / internal price audit | ✅ component odds + `/odds/bets` audit path | ✅ internal fair decimal/american + minimum playable price from #44 joint P | 🟠 v3.57 audit built | 🔴 final BET still requires executable combined quote | ⏳ | 🟠 BUILT / NATURAL AUDIT PENDING v3.57 | Internal SGP fair price is ours and already calculable. v3.57 audits the provider's full prematch bet catalog for explicit SGP/Bet Builder types. If none exists, component-product remains reference only and the actual sportsbook combined quote stays an external requirement. |
