...
class TacticalAnalyzer:
    ...
    def analyze_team_style(self, team_matches: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            possession_patterns = self.feature_engineering.analyze_possession_patterns(team_matches)
            
            # Análisis de pressing
            pressing_stats = self.feature_engineering.analyze_pressing_effectiveness(team_matches)
            
            # Análisis de ataque
            attacking_patterns = self.feature_engineering.analyze_attacking_patterns(team_matches)
            
            # Análisis defensivo
            defensive_patterns = self.feature_engineering.analyze_defensive_patterns(team_matches)
    ...
    def _analyze_attacking_matchup(self, team1_attack: Dict[str, float],
        ...
        return {
            ...
            'transition_effectiveness': team1_attack['transition_speed'] - team2_defense['defensive_action_rate'],
            'wing_play_effectiveness': team1_attack['wing_play'] - team2_defense['pressing_tendency'],
            'fluidity_vs_structure': team1_attack['positional_fluidity'] - (1 - team2_defense['defensive_line_height'])
        }
    ...
    def _calculate_tactical_advantages(self, team1_style: Dict[str, Any], 
        ...
        advantages['phases']['transitions'] = (
            ...
            team2_style['defensive_style']['defensive_action_rate']
        ) * 0.5
    ...
    def _identify_tactical_traits(self, style_profile: Dict[str, Any]) -> tuple:
        ...
        if style_profile['defensive_style']['defensive_line_height'] < 0.3:
            strengths.append("Bloque defensivo sólido")
        elif style_profile['defensive_style']['defensive_line_height'] > 0.7:
    ...
    def _calculate_tactical_indices(self, tactical_data: Dict[str, Any]) -> Dict[str, float]:
        ...
        if all(key in tactical_data for key in ['possession_style', 'pressing_style', 'attacking_style', 'defensive_style']):
            indices['tactical_flexibility_index'] = (
                tactical_data['possession_style'].get('pressure_resistance', 0.5) * 0.25 +
                tactical_data['pressing_style'].get('coordination', 0.5) * 0.25 +
                tactical_data['attacking_style'].get('positional_fluidity', 0.5) * 0.25 +
                tactical_data['defensive_style'].get('defensive_action_rate', 0.5) * 0.25
            )
    ...
    def _extract_defensive_features(self, match_data: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        # Promediar valores
        num_matches = max(1, len(match_data))
        features['defensive_line_height'] = defensive_line_height / num_matches
